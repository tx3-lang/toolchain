...
A thin aggregator of the Tx3 toolkit. Real code lives in five submodules:

- `tx3/` → [`tx3-lang/tx3`](https://github.com/tx3-lang/tx3) — language: parser, analyzer, TIR, codegen.
...
- `tx3/` — parser, type system, TIR, codegen, language semantics.
...
Dependency direction for cross-cutting changes is typically `tii` / `trp` → `tx3` → `trix` → `docs`.
